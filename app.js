...
function buildFilters() {
  ...
  const colors = ['All', ...new Set(PRODUCTS.map(product => product.color))];
...
function renderGrid() {
  ...
  const g = document.getElementById('grid');
  if (!g) return;
  const list = activeColor === 'All' ? PRODUCTS : PRODUCTS.filter(product => product.color === activeColor);
  g.innerHTML = '';
  list.forEach((product, idx) => {
    const card = document.createElement('div');
    card.className = 'pcard';
    const dotColor = DOTS[product.color] || 'var(--gold)';
    const priceDisplay = product.price ? '$' + Number(product.price).toFixed(2) : '—';
    card.innerHTML =
      '<div class="pcard-img-wrap">' +
        '<img class="pcard-img" src="' + product.image + '" alt="' + product.name + '" loading="lazy"/>' +
      '</div>' +
      (product.badge ? '<span class="pbadge">' + product.badge + '</span>' : '') +
      '<div class="pbody">' +
        '<p class="pcat"><span class="cdot" style="background:' + dotColor + '"></span>' + product.color + '</p>' +
        '<h3 class="pname">' + product.name + '</h3>' +
        '<p class="pcode">Size: ' + product.size + '</p>' +
        '<div class="pfoot">' +
          '<p class="pprice">' + priceDisplay + '</p>' +
...
function updCart() {
  ...
  el.innerHTML = cart.map(item =>
    ...
    '<div class="ci-inf">' +
      ...
      '<p class="ci-price">$' + ((Number(item.price) || 0) * item.qty).toFixed(2) + '</p>' +
      '<div class="ci-ctrl">' +
        '<button class="qbtn" onclick="chQ(' + item.id + ',-1)">&#x2212;</button>' +
        '<span class="qn">' + item.qty + '</span>' +
        '<button class="qbtn" onclick="chQ(' + item.id + ',1)">&#x2B;</button>' +
        '<button class="rmbtn" onclick="rmC(' + item.id + ')">Remove</button>' +
      '</div>' +
    '</div></div>'
  ).join('');
}
...
function sendWA() {
  ...
  const lines = cart.map(item =>
    ...
    (item.price ? ' = $' + ((Number(item.price) || 0) * item.qty).toFixed(2) : '')
  ).join('\n');
  const msg =
    'Hello Karyn Hair! 👋\n\n' +
    '*Name:* ' + name + (phone ? '\n*Phone:* ' + phone : '') +
...
(function init() {
  ...
  // Show home page
  go('home');
  // Build filters (products.js already loaded)
  buildFilters();
})();
